...
- [4.1.1 Enumeration](4.1.1%20Enumeration.md)

### File Permissions
Looking for sensitive files that is not usually open to basic users. A main target of interest being the `/etc/shadow` file.

Both `/etc/passwd` and `/etc/shadow` can be used with the `unshadow` tool to combine both files into one which can then be used for password bruteforcing.

### SSH Keys
Some environements can be utilizing SSH for remote login such as in servers and production environments. With that in mind, there can also be *private key files* which enables password-less login to the server.
