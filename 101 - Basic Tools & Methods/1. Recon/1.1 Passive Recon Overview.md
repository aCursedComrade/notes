#Basics #OSINT 
***

Gathering information on the target from the outside perspective.

### Physical/Social Assessments:
- Location Information
	- Satellite Images
	- Drone recon
	- Building layout (checkpoints, badge readers, break areas, etc.)

- Job/Person-specific Information
	- Employee related (names, job title, contact numbers, managers, etc)
	- Pictures (badge photo, desk photo, computer photos, etc)

### In Web/Host Assessments:
- Validating the target
- Finding subdomain and directory busting
- Fingerprinting (identifying technologies)
- Identifying data breaches

